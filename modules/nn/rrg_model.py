...
class RRGModel(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.adalabel_config = AdaLabelConfig(config.vocab_size,pad_token_id=0, bos_token_id=1,eos_token_id=102)
        self.bart = AdaLabelForConditionalGeneration(self.adalabel_config)
        self.criterion = AdaLabLoss(self.adalabel_config.vocab_size, 16, ignore_index=-100,temperature=config.ada_temp,eos_index=102)
    ...
    def forward(self, **kwargs):
        ...
        outs = self.bart(src_input_ids, src_attention_mask,
                                         decoder_input_ids = tgt_input_ids, decoder_attention_mask = tgt_attention_mask)
        ...
        loss = self.calc_loss(logits1_log_softmax, logits2, tgt_input_ids)
        
        pred_ids = torch.argmax(logits, dim=-1)
        padding_ids = 0
        no_padding = tgt_input_ids.ne(padding_ids)
        num_correct = pred_ids.eq(tgt_input_ids).masked_select(no_padding)
        correct = num_correct.sum().item()
        num = tgt_input_ids.ne(0).sum()
        acc = correct/num
    ...
    def calc_loss(self, generate1, generate2, decoder_input_ids = None):
        if decoder_input_ids is None:
            return None
        loss2 = F.cross_entropy(generate2.view(-1, self.adalabel_config.vocab_size), decoder_input_ids.view(-1), ignore_index=0)
        ada_loss = self.criterion(generate1.view(-1, self.adalabel_config.vocab_size), decoder_input_ids.view(-1), 
                                  decoder_input_ids, generate2.view(-1, self.adalabel_config.vocab_size))
        loss = loss2 + ada_loss
        return loss